...
from src.config.settings import (
    ...
    GRIMOIR_CHARGE_TIME, GRIMOIR_COOLDOWN,
)
...
class Player(pygame.sprite.Sprite):
    ...
    def _handle_weapons(self, dt, input_handler):
        ...
        if input_handler.is_scythe_pressed() and self.scythe_cooldown <= 0 and not self.scythe_active:
            ...
            self.scythe_cooldown = SCYTHE_COOLDOWN
    ...
    def is_scythe_hitting(self, target_pos):
        if not self.scythe_active:
            return False
        to_target = pygame.math.Vector2(target_pos) - pygame.math.Vector2(self.rect.center)
        if to_target.length() > SCYTHE_RANGE:
            return False
        target_angle = to_target.angle_to(pygame.math.Vector2(1, 0))
        diff = abs((target_angle - self.angle) % 360)
        return diff <= SCYTHE_ARC / 2 or (360 - diff) <= SCYTHE_ARC / 2
